...
const {id} = useParams()
    ...
    const handleBuyNow = useCallback(() => {
      addToCart(product._id);
      navigate("/cart");
    }, [addToCart, navigate, product?._id]);
    ...
    return product && (
        <div className="mt-12">
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-3">
                        {product.image.map((image, index) => (
                            ...
                            >
                                <LazyImage src={image} alt={`Thumbnail ${index + 1}`} />
                            </div>
                        ))}
                    </div>

                    <div className="border border-gray-500/30 max-w-100 rounded overflow-hidden">
                        <LazyImage src={thumbnail} alt="Selected product" className="w-full h-full object-cover" />
                    </div>
                </div>
          ...
          <div className="flex flex-col item-center mt-20">
            ...
            <button onClick={()=>{navigate('/products');scrollTo(0,0)}} className="mx-aupt cursor-pointer px-12 my-16 py-2.5 border
